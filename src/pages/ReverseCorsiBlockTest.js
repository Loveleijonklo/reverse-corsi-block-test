...
const INSTRUCTION_REPEAT = "Now repeat the pattern you saw by tapping the block in the reverse order"

function ReverseCorsiBlockTest() {
    ...
    useEffect(() => {
        ...
        if (answer[answer.length-1] === question[question.length - answer.length]) {
    ...
    useEffect(() => {
        if (isTestOver) {
            testProgress.addReverseCorsiBlockResult(score.current)
...
export default ReverseCorsiBlockTest
